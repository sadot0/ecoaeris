 ...
 * Файл: pricing.js
 * Страница: pricing.html (Аренда)
 * Автор: Мухаммад
 *
 * JS-функция: Chart.js график AQI за 7 дней для 3 городов Узбекистана
 * (Ташкент, Самарканд, Бухара) + PM2.5 -> AQI конвертер по формуле
 * US EPA с анимацией счётчика (ease-out-cubic).
 */

/*
 * Заметки разработчика (Мухаммад):
 *
 * Я отвечал за страницу аренды: тарифы, график AQI и PM2.5→AQI конвертер.
 *
 * График (Chart.js):
 * - Линейный график AQI за 7 дней по 3 городам (Ташкент, Самарканд, Бухара)
 * - Данные демонстрационные — бесплатный тариф WAQI не даёт историю
 * - Градиентная заливка под линией Ташкента для визуального акцента
 *
 * PM2.5→AQI конвертер:
 * - Формула US EPA: кусочно-линейная интерполяция по 6 диапазонам (breakpoints)
 * - Сначала делал как aqi = pm25 * 5, но это неправильно
 * - Изучил Википедию и переделал через таблицу breakpoints
 * - Анимация счётчика через requestAnimationFrame с ease-out-cubic
 *
 * Что было сложно:
 * - Chart.js прыгал при ресайзе — решил через maintainAspectRatio: false
 * - Конвертер: зазоры между диапазонами (9.0 и 9.1) — закрыл непрерывными границами
 *
 * Что я узнал: Chart.js, математические формулы в JS, requestAnimationFrame
 */
...
(function () {
  ...
  const EPA_BREAKPOINTS = [
    ...
    [9.0,  35.4,  51,  100, 'Умеренный',         '#FFFF00'],
    [35.4, 55.4,  101, 150, 'Вредный для уязвимых', '#FF7E00'],
    [55.4, 125.4, 151, 200, 'Вредный',           '#FF0000'],
    [125.4, 225.4, 201, 300, 'Очень вредный',    '#8F3F97'],
    [225.4, 500,  301, 500, 'Опасный',           '#7E0023'],
  ];
