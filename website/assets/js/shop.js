...
/*
 * Заметки разработчика (Самира):
 *
 * Я делала магазин с каталогом, фильтрами и корзиной.
 * Уникальная фича — корзина сохраняется в localStorage.
 *
 * Как работают фильтры:
 * - Бренды: ИЛИ (Xiaomi ИЛИ Philips показывает оба)
 * - Цена + покрытие + бренды: И (пересечение всех фильтров)
 * - Сортировка: по цене или площади покрытия
 *
 * Что было сложно:
 * - localStorage — при обновлении страницы корзина терялась, решила через
 *   JSON.parse/stringify с try-catch на случай битых данных
 * - Фильтры с логикой ИЛИ внутри группы и И между группами —
 *   пришлось разделить getActiveFilters() и applyFilters()
 * - Плавающая корзина: сначала использовала position:right, потом
 *   переделала на translateX для плавной анимации
 *
 * Что я узнала: localStorage API, динамический рендеринг DOM, event delegation
 */
