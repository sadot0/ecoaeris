...
/*
 * Заметки разработчика (Муслима):
 *
 * Моя страница — калькулятор подбора очистителя воздуха.
 * Пользователь отвечает на 4 вопроса, и алгоритм рекомендует лучшую модель.
 *
 * Как работает алгоритм:
 * - Все 12 моделей получают баллы по 4 критериям: покрытие (35%), бюджет (25%),
 *   потребности (25%), тип помещения (15%)
 * - Ни одна модель не отсеивается — все оцениваются и сортируются по скору
 * - Результат показывает % совпадения и 3 альтернативы
 *
 * Что было сложно:
 * - Wizard с автопереходом — сначала переключался слишком быстро,
 *   добавила setTimeout 300мс чтобы пользователь увидел свой выбор
 * - Формула скоринга — перебрала 3 варианта пока не нашла баланс
 * - Адаптив: на мобильном 6 карточек потребностей в 2 колонки выглядели мелко,
 *   переделала на 1 колонку через media query
 *
 * Что я узнала: DOM-манипуляции, CSS-анимации, алгоритмы скоринга
 */
